list.of.packages <- c("data.table", "rjson", "stringr")
...
#"""load data files from a directory that contains all the compressed (or uncompressed) data files straight off the station"""
  ...
  dfs <- function(x,y,z=NULL) {
    ...
    known <- c("Time","RadioId","TagId","TagRSSI","NodeId","Validated","NodeRSSI","Battery","Celsius","RecordedAt", "Firmware","SolarVolts","SolarCurrent","CumulativeSolarCurrent","Latitude","Longitude","recorded.at",
               "gps.at","latitude","longitude","altitude","quality","mean.lat","mean.lng","n.fixes", "UsbPort", "Sync", "Product", "Revision", "VCC", "Temp", "Raw.Payload")
    ...
    listdf <- lapply(x, function(i) { #cols=NULL
      ...
      ss <- unlist(strsplit(basename(i), "-"))[2]
      df <- tryCatch({
      if (file.size(i) > 0) {
      ...
      if(is.data.frame(df)) {
        ...
        if (y=="beep" & v < 2) {
          ...
          if(any(indx != correct)) {
            ...
            getrow <- read.csv(i,as.is=TRUE, na.strings=c("NA", ""), skipNul = TRUE, skip=rowfix-1, nrow=1)
            getrow[,1]<- str_extract(getrow[,1], "\\d{4}-.*")
            #df[rowfix,] <- getrow[1,]
            df <- df[-rowfix,]
        }
        } else if (y == "beep" & v > 1) {
...
#this also converts Time to POSIXct, removes records that have NA time or don't fit the format
  ...
  ble <- list.files(directory_name, pattern = ble_pattern, full.names = TRUE, recursive = TRUE)
  len <- length(ble)
  if (len > 0) {
    print(paste("preparing",len,"ble files for merge"))
    version <- 2
    ble_dataset <- df_merge(ble,z=version, starttime = starttime, endtime = endtime)
    ble_data <- ble_dataset[[1]]
  } else { 
    print("no gps files found in directory")
    ble_dataset <- list(data.frame(), NA)
    ble_data <- data.frame()}
  
  if (!is.null(tags) & !is.null(beep_data) & any(tags %in% beep_data$TagId)) {beep_data <- beep_data[beep_data$TagId %in% tags,]}
  lst <- list(list(beep_data, beep_dataset[[2]]), health_dataset, gps_dataset, ble_dataset)
  names(lst) <- c("beep", "node", "gps", "ble")
return(lst)}
